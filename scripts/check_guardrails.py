...
對應 .kiro/steering/guardrails.md 中可「靜態」驗證的性質 P1-P5。
...
ALLOWED_PORT = 8080
SKIP_PARTS = {".git", ".kiro", "target", "__pycache__", "venv", ".venv", "node_modules"}
...
def collect_targets(argv: list):
    ...
    return python_files, web_files, html_files, js_files


def main(argv: list) -> int:
    python_files, web_files, html_files, js_files = collect_targets(argv)
    violations: list = []
    ...
    scanned = len(python_files) + len(web_files)
    if not violations:
        print("紅線檢查通過：掃描 {0} 個檔案，未發現 P1-P5 違規。".format(scanned))
        return 0
    ...
    for violation in sorted(violations, key=lambda v: (v.rule, str(v.path), v.line)):
        print("  " + str(violation))
    print("")
    print("修正指引：P5 直接補 encoding='utf-8'；P1／P2／P3／P4 屬架構紅線，")
    print("不得自行變更架構，請回報 System Integrator。")
    return 1
